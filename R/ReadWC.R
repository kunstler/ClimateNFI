...
read_wc_stack_rad <- function(var = 'srad',  
  ...
  layers <- rast(file.path(path, files_sel[1]))
  for (m in 2:12){
    layer_t <- rast(file.path(path, files_sel[m]))
    add(layers) <- layer_t
  }
...
extract_wc_var <- function(coords, var = "srad"){
  ...
  write.csv(bind_cols(coords, as.data.frame(res[, -1])), file = file.path("output", paste0("wc_",var, ".csv")), row.names = FALSE)
